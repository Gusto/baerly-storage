...
| `db.compaction.below_min_total` sustained on a scheduled loop | `compact()` skipped: live tail below `minEntriesToCompact` (and no fold budget checkpoint) | With the scheduled default floor of 1 this is ordinary idling (nothing to fold). If your cron passes an explicit threshold, a sustained rate means the tail sits in the dead zone below it — every read replays those entries at one GET apiece. Raise the cron cadence, lower the threshold, or fold more per pass. |
