...
- **The log-retention sequence window as a paused-writer fence.** Withdrawn.
  `LOG_RETENTION_SEQ_WINDOW` bounds no wall-clock and no commit count, so a
  delayed create PUT, an isolate suspension, or the writer's own transient-retry
  loop can each straddle an unbounded number of peer commits and folds. The
  commit-path check (`assertCommitAboveDeleteFloor` in invariant 5) closes that
  schedule instead. The window survives as a pre-image cost margin and a retirement
  rate limit, with no safety property attached.
- **Post-create manifest revalidation with discard-and-re-probe.** Insufficient,
  and for a stronger reason than the per-commit GET cost. Discard-and-re-probe
  cannot distinguish "my create landed and was folded" from "a foreign create
  landed and was folded": the fold retains no writer identity, so the two
  histories leave identical durable state and demand opposite outcomes. The
  replacement triggers on certified deletion (`seq < log_delete_floor`) and fails
  with `AmbiguousCommit` rather than attempting indeterminate recovery. A
  pre-create re-read closes neither schedule — a GET cannot constrain a later
  PUT to a different key, and the lost-ack arm must not re-read at all without
  breaking own-session adoption.
