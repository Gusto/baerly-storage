...
SigV4 implementations). Define our own subset of the S3 wire types in
`src/s3-types.ts` instead of pulling them from the SDK. Parse XML
responses with `@xmldom/xmldom` (already needed for ListObjectsV2;
re-using for everything S3 returns).
