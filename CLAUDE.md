...
- **`docs/api/` is generated.** Regenerated by `pnpm doc` (typedoc). Don't
  hand-edit; edit JSDoc on the source instead.
...
- ❌ Hand-editing `docs/api/`. Run `pnpm doc` instead.
...
- **New public API method on `MPS3`?** See [docs/EXTENDING.md](docs/EXTENDING.md).
  Add JSDoc with `@example` — typedoc consumes it.
