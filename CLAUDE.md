...
| `pnpm verify:examples` | runs each scaffoldable example's `tsc -b --noEmit` (`minimal-cloudflare`, `minimal-node`, `helpdesk-cloudflare`) so SPA + Worker bugs in the templates fail fast | ~seconds | ✅ |
...
| `cat node_modules/baerly-storage/dist/AGENTS.md` | 367-line public-API quickref. Read this BEFORE walking the hash-suffixed `dist/*.d.ts` chain. Source lives at `packages/server/AGENTS.md`; the rolldown `closeBundle` step copies it to `dist/AGENTS.md` on every build | n/a | ✅ |
...
11. **`examples/`** — runnable example apps that double as the CLI
   ...
   and `examples/helpdesk-cloudflare/` are the production-shaped
   scaffolds; `examples/helpdesk/` is a dev-only teaching fixture:
   single Vite process; the Baerly HTTP listener is mounted as
   middleware via `baerlyDev()` from `@baerly/dev/vite`; ticket
   CRUD over `LocalFsStorage`. Each scaffoldable example carries
