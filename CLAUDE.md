...
| `pnpm verify:docs`                                                      | `verify-docs.mjs` (frontmatter `related:` cross-link resolver + `audience:` field audit + 180-day `last-reviewed:` staleness check) **+ `remark-validate-links`** (inline Markdown link + heading-anchor validation across all of `docs/`, runnable standalone as `verify:doc-links`) **+ `verify-mermaid.mjs`** (parses every fenced ` ```mermaid ` block in tracked Markdown with the real `mermaid` engine under a happy-dom shim — the parser GitHub uses; catches unquoted-label syntax errors `remark-validate-links` can't see; standalone as `verify:mermaid`). Runs as part of `pnpm verify`.                                                                                                                                                  | ~seconds                             | ✅                                                                                                                                                                    |
