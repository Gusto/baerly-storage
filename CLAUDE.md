...
| ----------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `pnpm verify`                                                           | typecheck (`tsgo --noEmit`) + `verify:examples` + lint (`oxlint`) + `format:check` (`oxfmt --check .`, whole-repo) + `lint-format-coverage` (ownership guard) + `verify:docs` (markdown validation)                                                                                                                                                                                                                                                                                                                      | ~seconds                             | ✅ — non-zero exit _is_ your regression                                                                                                                               |
...
| `pnpm verify:docs`                                                      | `verify-docs.mjs` (frontmatter `related:` cross-link resolver + `audience:` field audit + 180-day `last-reviewed:` staleness check — scoped to `docs/`, where that frontmatter convention lives) **+ `remark-validate-links`** (inline Markdown link + heading-anchor validation across **all tracked Markdown** — `docs/`, root `README`/`CLAUDE`/`AGENTS`, `packages/**/API.md`, examples — runnable standalone as `verify:doc-links`) **+ `verify-mermaid.mjs`** (parses every fenced ` ```mermaid ` block in tracked Markdown with the real `mermaid` engine under a happy-dom shim — the parser GitHub uses; catches unquoted-label syntax errors `remark-validate-links` can't see; standalone as `verify:mermaid`). Runs as part of `pnpm verify`.                                                                                                                                                  | ~seconds                             | ✅                                                                                                                                                                    |
