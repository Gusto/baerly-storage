...
export async function replicate(
  ...
  const source_replication = await sha256b64(sourceManifest.ref.bucket + sourceManifest.ref.key);
  ...
  await manifestUpdate.then((delta: ManifestFile) => {
    for (const [url, state] of Object.entries(delta.files)) {
      ...
      if (inside(target_replication, state.replication || <b64>"")) continue; // already synced, no action
      ...
      perKeyOptions.set(sourceRef.key, {
        replication: or(source_replication, state.replication || <b64>""),
      });
    }
  });
