...
| `bench/measurement/workload-ceiling-arm-symmetry-run.ts` | Paired Node CPU probe for the workload-ceiling control and chunked candidate: complete-read hash/decode/canonical-re-encode cost plus each arm's 20-mutation fold, with candidate-minus-control net cost. Workerd figures are visibly marked inherited, unverified conversions rather than measurements. No infra. MEASURES ONLY; chooses no admission margin. | Before deciding the locality-specific workload-ceiling admission margins |
...
- **Every emitted number is `-0`-normalized**, echoed caller parameters
  included. `-0` clears both `Number.isInteger(v) && v >= 0` and
  `!(v < 0)` untouched, so normalizing keeps emitted records
  `Object.is`-comparable and snapshot-stable. (The canonical JSON these
  results feed normalizes `-0` to `0` as well, so this is belt-and-braces
  on the serialization side rather than a hard requirement of it.)
...
per trigger. Its content objects are explicitly seeded legacy load, so they
exercise retained collector work rather than current-writer production.
