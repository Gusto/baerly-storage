 ...
 * account's bucket, `workload-ceiling-provision-sweep.ts` calibrates and
 * provisions every cell into it, `bench/workload-ceiling-worker/deploy.mjs`
 * deploys the study Worker into it, and `workload-ceiling-collect.ts`
 * queries its telemetry. `workload-ceiling-smoke.ts` forwards the variable
 * to the collector it spawns rather than resolving it itself, so it cannot
 * disagree. A value none of them recognizes has to stop the run rather than
 * resolve to one:
