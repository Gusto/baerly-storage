...
- `packages/create-baerly-storage/templates/addons/<name>/` — opt-in overlays
  ...
  .dockerignore; requires `--target=node`). Agent rules are generated by
  `src/agent-rules.ts`, not copied from this overlay tree.
...
`@baerly/cli/wrangler-patch`, then runs five core writes in order — each
gated on skip-if-exists or merge-if-present semantics. The CLI also
passes `agentRules: true` by default, so a managed agent-rules block is
created, appended, or replaced unless the user passes `--no-agent-rules`.
