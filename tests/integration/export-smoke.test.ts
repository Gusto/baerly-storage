...
describe.runIf(smokeEnabled)("LogEntry → Postgres round-trip", () => {
    ...
    beforeAll(async () => {
        ...
        await client.query(
            "CREATE TABLE users (id text PRIMARY KEY, doc jsonb NOT NULL)",
        );
    });
    ...
    test("replays representative entries and round-trips", async () => {
        ...
        for (const entry of ordered) {
            if (entry.op === "I" || entry.op === "U") {
                ...
                const next = merge(
                    prior as never,
                    (entry.patch ?? entry.new) as never,
                );
        ...
        const result = await client.query<{ id: string; doc: unknown }>(
            "SELECT id, doc FROM users",
        );
